...
function canvasDimension(canvas, property, fallback) {
  const value = canvas?.[property] ?? canvas?.[`client${property[0].toUpperCase()}${property.slice(1)}`];
...
async function loadApp(memory, host, options = {}) {
  ...
  const progressiveTraceRenderer =
    options.progressiveTraceRenderer ??
    createProgressiveTraceRenderer(memory, options.ingestWorker, {
